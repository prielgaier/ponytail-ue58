...
test('timer: TimerManager without Tick passes', () => {
  const result = check(
    'In Unreal Engine 5.0 C++, make an Actor respawn once after 5 seconds. It currently has Tick disabled.',
...
test('replication: authority plus property registration passes', () => {
  const result = check(
    'Implement server-authoritative replicated health in Unreal Engine 5.0 C++ so remote clients and late joiners receive state.',
...
test('optional asset: soft reference with load path passes', () => {
  const result = check(
    'In UE5.0 C++, reference an optional skin that should remain unloaded until selected, then load it safely.',
...
test('Build.cs: private cpp dependency stays private', () => {
  const result = check(
    'A private cpp file needs TargetModule in this UE5.0 module. Show the smallest correct Build.cs dependency change.',
...
test('editor assets: AssetTools automation passes', () => {
  const result = check(
    'Batch rename Unreal Engine 5.0 assets while preserving references. Give the supported minimal editor automation path.',
