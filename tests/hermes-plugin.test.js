...
`, { XDG_CONFIG_HOME: tmp });
  ...
  assert.match(ctx, /PONYTAIL UE5\.6 MODE ACTIVE — level: ultra/);
...
`);
  const { ctx } = JSON.parse(output);
  assert.match(ctx, /PONYTAIL UE5\.6 MODE ACTIVE — level: review/);
  assert.match(ctx, /Ponytail UE5\.6 review/);
...
`);
  ...
  assert.match(data.context, /PONYTAIL UE5\.6 MODE ACTIVE — level: ultra/);
});
