...
`, { XDG_CONFIG_HOME: tmp });
  ...
  assert.match(ctx, /PONYTAIL UE4\.27 MODE ACTIVE — level: ultra/);
...
`);
  const { ctx } = JSON.parse(output);
  assert.match(ctx, /PONYTAIL UE4\.27 MODE ACTIVE — level: review/);
  assert.match(ctx, /Ponytail UE4\.27 review/);
...
`);
  ...
  assert.match(data.context, /PONYTAIL UE4\.27 MODE ACTIVE — level: ultra/);
});
