...
assert.equal(output.systemMessage, 'PONYTAIL UE4.27:ULTRA');
...
assert.match(
  ...
  /PONYTAIL UE4\.27 MODE ACTIVE — level: ultra/,
);
...
assert.equal(output.systemMessage, 'PONYTAIL UE4.27:LITE');
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE4\.27 MODE ACTIVE — level: lite/,
);
...
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'PONYTAIL UE4.27:OFF');
...
output = JSON.parse(result.stdout);
assert.match(output.additionalContext, /PONYTAIL UE4\.27 MODE ACTIVE — level: full/);
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE4\.27 MODE ACTIVE — level: full/,
);
...
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'PONYTAIL UE4.27:FULL');
assert.equal(output.additionalContext, undefined, 'Codex must not emit additionalContext at top level (#573)');
assert.equal(output.hookSpecificOutput.hookEventName, 'SubagentStart');
assert.match(output.hookSpecificOutput.additionalContext, /PONYTAIL UE4\.27 MODE ACTIVE — level: full/);
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE4\.27 MODE CHANGED — level: ultra/,
);
...
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.additionalContext, 'PONYTAIL UE4.27 MODE OFF');
