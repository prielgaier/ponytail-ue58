...
assert.equal(output.systemMessage, 'PONYTAIL UE5.0:ULTRA');
...
assert.match(
  ...
  /PONYTAIL UE5\.0 MODE ACTIVE — level: ultra/,
);
...
assert.equal(output.systemMessage, 'PONYTAIL UE5.0:LITE');
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE5\.0 MODE ACTIVE — level: lite/,
);
...
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'PONYTAIL UE5.0:OFF');
...
output = JSON.parse(result.stdout);
assert.match(output.additionalContext, /PONYTAIL UE5\.0 MODE ACTIVE — level: full/);
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE5\.0 MODE ACTIVE — level: full/,
);
...
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'PONYTAIL UE5.0:FULL');
assert.equal(output.additionalContext, undefined, 'Codex must not emit additionalContext at top level (#573)');
assert.equal(output.hookSpecificOutput.hookEventName, 'SubagentStart');
assert.match(output.hookSpecificOutput.additionalContext, /PONYTAIL UE5\.0 MODE ACTIVE — level: full/);
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /PONYTAIL UE5\.0 MODE CHANGED — level: ultra/,
);
...
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.additionalContext, 'PONYTAIL UE5.0 MODE OFF');
