"""Hermes plugin for Ponytail UE5.0."""
...
SKILL_COMMANDS = {
    "ponytail-review": "Review the current UE5.0 diff for avoidable engine and project complexity.",
    "ponytail-audit": "Audit the UE5.0 repo for unnecessary systems, Tick, reflection, and dependencies.",
    "ponytail-debt": "List each deliberate `ponytail:` shortcut with its trigger and verification boundary.",
    "ponytail-gain": "Summarize measurable simplification in the current UE5.0 diff.",
    "ponytail-help": "Show the Ponytail UE5.0 command and safety reference.",
}
...
def _fallback_instructions(mode: str) -> str:
    return (
        f"PONYTAIL UE5.0 MODE ACTIVE — level: {mode}\n\n"
        "Make the smallest correct Unreal Engine 5.0 change. Inspect the .uproject, "
...
def build_injected_context(mode: str | None = None) -> str:
    ...
    if configured == "review":
        try:
            ...
            return f"PONYTAIL UE5.0 MODE ACTIVE — level: review\n\n{_strip_frontmatter(body)}"
        except OSError:
            return "PONYTAIL UE5.0 MODE ACTIVE — level: review. Review UE5.0 diffs for unnecessary complexity."

    effective = _normalize_runtime_mode(configured) or DEFAULT_MODE
    try:
        body = PONYTAIL_SKILL.read_text(encoding="utf-8")
        return f"PONYTAIL UE5.0 MODE ACTIVE — level: {effective}\n\n{_filter_skill_body_for_mode(body, effective)}"
    except OSError:
...
def register(ctx: Any) -> None:
    ...
    ctx.register_command(
        ...
        description="Set Ponytail UE5.0 mode: lite, full, ultra, or off.",
