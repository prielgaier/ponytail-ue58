#!/usr/bin/env node
// Shared Ponytail UE5.3 instruction builder for Claude hooks and adapters.
...
function getFallbackInstructions(mode) {
  return 'PONYTAIL UE5.3 MODE ACTIVE — level: ' + mode + '\n\n' +
    'You are a lazy Unreal Engine 5.3 senior. Lazy means efficient, not careless. The best UObject is the one the project did not need.\n\n' +
    '## Persistence\n\n' +
    'ACTIVE ON EVERY UE5.3 RESPONSE. Do not apply to non-Unreal tasks. Off only: "stop ponytail" / "normal mode".\n\n' +
    ...
    '3. Does UE5.3 already do it? Prefer delegates, timers, notifies, Subsystems, Asset Manager, SaveGame, Enhanced Input, replication, navigation, Gameplay Tags, or an existing framework.\n' +
    ...
    '## Boundaries\n\n' +
    'Ponytail governs UE5.3 work, not non-Unreal tasks or how the user talks. "stop ponytail" or "normal mode": revert. Level persists until changed or session end.';
}
...
function getPonytailInstructions(mode) {
  ...
  if (INDEPENDENT_MODES.has(configuredMode)) {
    return 'PONYTAIL UE5.3 MODE ACTIVE — level: ' + configuredMode + '. Behavior defined by /ponytail-' + configuredMode + ' skill.';
  }

  const effectiveMode = normalizeMode(configuredMode) || DEFAULT_MODE;

  try {
    return 'PONYTAIL UE5.3 MODE ACTIVE — level: ' + effectiveMode + '\n\n' +
