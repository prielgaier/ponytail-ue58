    ...
    if (/^[/@$]ponytail/.test(prompt)) {
      ...
      if (cmd === '/ponytail-review' || cmd === '/ponytail:ponytail-review') {
        ...
        if (arg === 'default') {
          ...
          if (dmode === 'off' || dmode === 'lite' || dmode === 'full' || dmode === 'ultra') {
            ...
            writeHookOutput('UserPromptSubmit', dmode, 'PONYTAIL UE5.0 DEFAULT SET — new sessions start in ' + dmode + '.');
          }
      ...
      if (isReportOnly) {
        writeHookOutput(
          'UserPromptSubmit',
          mode,
          'PONYTAIL UE5.0 MODE ACTIVE — level: ' + mode,
        );
        ...
        if (!isQoder) {
          writeHookOutput(
            'UserPromptSubmit',
            mode,
            'PONYTAIL UE5.0 MODE CHANGED — level: ' + mode,
          );
        }
      } else if (mode === 'off') {
        clearMode();
        deactivated = true;
        writeHookOutput('UserPromptSubmit', 'off', 'PONYTAIL UE5.0 MODE OFF');
      }
    }
    ...
    if (isQoder && !deactivated) {
      ...
      if (currentMode && currentMode !== 'off') {
        ...
        const header = modeSwitched
          ? 'PONYTAIL UE5.0 MODE CHANGED — level: ' + currentMode + '\n\n'
          : '';
