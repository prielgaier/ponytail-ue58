...
test("/ponytail updates session mode and injects instructions", async () => withTempConfig(async () => {
  ...
  assert.ok(result.systemPrompt.includes("PONYTAIL UE5.2 MODE ACTIVE"));
...
test("before_agent_start guards missing event and missing systemPrompt (#439, #440)", async () => withTempConfig(async () => {
  ...
  for (const bad of [undefined, null]) {
    ...
    assert.ok(r.systemPrompt.includes("PONYTAIL UE5.2 MODE ACTIVE"));
  ...
  assert.ok(empty.systemPrompt.includes("PONYTAIL UE5.2 MODE ACTIVE"));
  ...
  assert.ok(withBase.systemPrompt.includes("PONYTAIL UE5.2 MODE ACTIVE"));
}));
...
test("a request mentioning normal mode stays active", async () => withTempConfig(async () => {
  ...
  const result = await events.get("before_agent_start")({ systemPrompt: "BASE" }, ctx);
  assert.match(result.systemPrompt, /PONYTAIL UE5\.2 MODE ACTIVE/);
}));
...
test("PONYTAIL_HIDE_STATUS hides the indicator but keeps ponytail active (#324)", async () => withTempConfig(async () => {
  ...
  assert.match(injected.systemPrompt, /PONYTAIL UE5\.2 MODE ACTIVE/, "ruleset must still inject while status is hidden");
}));
