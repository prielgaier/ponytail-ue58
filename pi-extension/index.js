...
export default function ponytailExtension(pi) {
  ...
  function syncStatus(ctx) {
    ...
    c.ui.setStatus("ponytail", indicator + " 🐴 " + theme.fg("muted", "ponytail UE5.3: ") + theme.fg("text", icon + " " + label));
  }
  ...
  const setMode = (mode, ctx) => {
    ...
    syncStatus(ctx);
    ctx?.ui?.notify?.(`Ponytail UE5.3 mode set to ${normalized}.`, "info");
  };
  ...
  pi.registerCommand("ponytail", {
    ...
    handler: async (args, ctx) => {
      ...
      if (parsed.type === "status") {
        ctx?.ui?.notify?.(`Ponytail UE5.3: current ${currentMode} • default ${configuredDefaultMode}`, "info");
        return;
      }
        ...
        try {
          ...
          if (written) {
            ...
            const message = configuredDefaultMode === written
              ? `Default Ponytail UE5.3 mode set to ${written}.`
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    syncStatus(ctx);
    if (!getQuietStartup()) {
      ctx?.ui?.notify?.(`Ponytail UE5.3 loaded: ${currentMode}`, "info");
    }
  });
