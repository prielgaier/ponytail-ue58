...
description: >
  Review an Unreal Engine 4.27 diff exclusively for unnecessary architecture,
  Tick/polling, reflection surface, modules/plugins, wrappers, components,
  duplicate engine functionality, and unsafe deletion assumptions. Use for UE4.27
  over-engineering reviews, simplify reviews, delete lists, or
  /ponytail-review. Report only; do not apply fixes.
---

# Ponytail UE4.27 review

Review the current UE4.27 diff for removable complexity after reading the
...
- `native:` custom code duplicating a UE4.27 facility already available.
...
If nothing is safely removable: `Lean for UE4.27 already. Ship.`
