...
description: >
  Review an Unreal Engine 5.6 diff exclusively for unnecessary architecture,
  Tick/polling, reflection surface, modules/plugins, wrappers, components,
  duplicate engine functionality, and unsafe deletion assumptions. Use for UE5.6
  over-engineering reviews, simplify reviews, delete lists, or
  /ponytail-review. Report only; do not apply fixes.
---

# Ponytail UE5.6 review

Review the current UE5.6 diff for removable complexity after reading the
...
- `native:` custom code duplicating a UE5.6 facility already available.
...
If nothing is safely removable: `Lean for UE5.6 already. Ship.`
