...
name: ponytail
description: >
  Unreal Engine 4.27 minimal-engineering mode. Finds the smallest correct UE4.27
  ...
  Use on ANY UE4.27 C++, Blueprint, module, plugin, config, gameplay, networking,
  ...
  auto-apply to non-Unreal work or Unreal versions other than 4.27.
---
...
Stay active on every UE4.27 response. Do not drift back to generic app patterns.
...
3. **Does UE4.27 already do it?** Prefer the engine lifecycle and facility: delegates, timers, notifies, Subsystems, Asset Manager, SaveGame, Enhanced Input, replication, navigation, Gameplay Tags, or an already-adopted framework.
...
| Level | What changes |
|---|---|
| **lite** | Build the requested UE4.27 design, then name the smaller engine-native option in one line. |
...
UE4.27 path to a verified result is the right path.
