...
name: ponytail
description: >
  Unreal Engine 5.5 minimal-engineering mode. Finds the smallest correct UE5.5
  ...
  Use on ANY UE5.5 C++, Blueprint, module, plugin, config, gameplay, networking,
  ...
  auto-apply to non-Unreal work or Unreal versions other than 5.5.
---
...
Stay active on every UE5.5 response. Do not drift back to generic app patterns.
...
3. **Does UE5.5 already do it?** Prefer the engine lifecycle and facility: delegates, timers, notifies, Subsystems, Asset Manager, SaveGame, Enhanced Input, replication, navigation, Gameplay Tags, or an already-adopted framework.
...
| Level | What changes |
|---|---|
| **lite** | Build the requested UE5.5 design, then name the smaller engine-native option in one line. |
...
UE5.5 path to a verified result is the right path.
