...
name: ponytail-audit
description: >
  Audit an entire Unreal Engine 5.2 repository for avoidable Tick, custom
  ...
  code or assets. Use for repo-wide UE5.2 complexity audits, bloat audits,
  deletion candidates, or /ponytail-audit. Report only; do not apply fixes.
---

# Ponytail UE5.2 audit

Audit the complete UE5.2 project, not only the diff. Read the `.uproject`,
...
- Wrappers around UE5.2 facilities already used elsewhere in the project.
...
If nothing is safely removable: `Lean for UE5.2 already. Ship.` Report only.
